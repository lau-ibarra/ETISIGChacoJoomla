...
class PhocaDownloadDownload
{
	...
	function getDownloadData($id, $return) {
		...
		if ((int)$limitEnabled > 0) {
			if ((int)$user->id < 1) {
				$app->redirect(JRoute::_($returnUrl), JText::_("COM_PHOCADOWNLOAD_NOT_LOGGED_IN_USERS_NOT_ALLOWED_DOWNLOAD"));
				exit;
			}
			...
			if ((int)$userFileCount > (int)$limitEnabled) {
				$app->redirect(JRoute::_($returnUrl), JText::_("COM_PHOCADOWNLOAD_MAX_LIMIT_DOWNLOAD_PER_FILE_REACHED"));
				exit;
			}
		}
