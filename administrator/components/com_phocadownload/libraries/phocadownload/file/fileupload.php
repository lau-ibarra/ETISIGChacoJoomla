...
class PhocaDownloadFileUpload
{
	...
	//	$uploadMethod 	= $paramsC->get( 'multiple_upload_method', 1 );
		...
		if (isset($file['name'])) {
			...
			if (!PhocaDownloadFileUpload::canUpload( $file, $errUploadMsg, $manager, $frontEnd )) {
				...
				if ($return) {
					$app->redirect(base64_decode($return).'&manager='.(string)$manager.'&folder='.$folderUrl, $errUploadMsg, 'error');
					exit;
				} else {
			...
			if (JFile::exists($filepath) && $overwriteExistingFiles == 0) {
				if ($return) {
					...
					$app->redirect($componentUrl, JText::_('COM_PHOCADOWNLOAD_FILE_ALREADY_EXISTS'), 'error');
					exit;
				}
			}
			...
			if (!JFile::upload($file['tmp_name'], $filepath)) {
				if ($return) {
					$app->redirect(base64_decode($return).'&manager='.(string)$manager.'&folder='.$folderUrl, JText::_('COM_PHOCADOWNLOAD_ERROR_UNABLE_TO_UPLOAD_FILE'), 'error');
					exit;
				} else {
					$app->redirect($componentUrl, JText::_('COM_PHOCADOWNLOAD_ERROR_UNABLE_TO_UPLOAD_FILE'), 'error');
					exit;
				}
			} else {
				...
				if ($return) {
					$app->redirect(base64_decode($return).'&manager='.(string)$manager.'&folder='.$folderUrl, JText::_('COM_PHOCADOWNLOAD_SUCCESS_FILE_UPLOAD'));
					exit;
				} else {
					$app->redirect($componentUrl, JText::_('COM_PHOCADOWNLOAD_SUCCESS_FILE_UPLOAD'));
					exit;
				}
			}
		} else {
			...
			if ($return) {
				$app->redirect(base64_decode($return).'&manager='.(string)$manager.'&folder='.$folderUrl, $msg);
				exit;
			} else {
				if($viewBack != '') {
					...
					$app->redirect('index.php?option=com_phocadownload', $msg, 'error');
				}
	...
	function canUpload( $file, &$err, $manager = '', $frontEnd = 0, $chunkEnabled = 0, $realSize = 0) {
