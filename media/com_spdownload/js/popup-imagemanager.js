...
var ImageManager = this.ImageManager = {
	initialize: function()
	{
		...
		this.fields.caption	= document.id("f_caption");
                this.fields.icon	= document.id("f_icon");
                this.fields.name	= document.id("f_name");
                this.fields.type	= document.id("f_type");
	...
	onok: function()
	{
		...
		var caption	= this.fields.caption.get('value');
                var icon	= this.fields.icon.get('value');
                var name	= this.fields.name.get('value');
                var type	= this.fields.type.get('value');
		...
		if (url != '') {
			...
			var tag = "<img src=\""+url+"\" "+extra+"/>";
                        ...
                        var tag;
                        if (type == 'doc') {
                            tag = '<a href="'+url+'"><img src="'+icon+'" border="0" /><span title="'+name+'">'+name+'</span></a>';
                        } else {
	...
	setFolder: function(folder,asset,author)
	{
		...
		this.frame.location.href='index.php?option=com_spdownload&view=imagesList&tmpl=component&folder=' + folder + '&asset=' + asset + '&author=' + author;
	},
	...
	populateFields_old: function(file)
	{
		document.id("f_url").value = image_base_path+file;
	},
        
        populateFields: function(file, icon, name, type)
	{
            	document.id("f_url").value = image_base_path+file;
                document.id("f_icon").value = icon;
                document.id("f_name").value = name;
                document.id("f_type").value = type;
	},
