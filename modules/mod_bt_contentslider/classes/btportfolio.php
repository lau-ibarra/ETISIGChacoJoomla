	...
	public function getList() {
		...
		foreach ($data as $key => &$item) {
			...
			if (in_array($item->access, $user->getAuthorisedViewLevels())) {
				...
				$item->link = JRoute::_("index.php?option=com_bt_portfolio&view=portfolio&id=" . $item->id .':'.$item->alias .'&catid_rel='.$item->catid.':'.$item->category_alias);
			}
			else {
			...
			$item->categoryLink = JRoute::_("index.php?option=com_bt_portfolio&view=portfolios&catid=" . $item->catid .':'.$item->category_alias);
			...
			if ($params->get('show_image')) {
				...
				if (!$image) {
					...
					$url_image = JURI::root() . 'images/bt_portfolio/' . $item->id . '/large/' . $image;
				}
