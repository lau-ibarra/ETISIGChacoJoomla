...
<?php } ?>
        ...
        <div class="maxiroundedcenter">
            <ul class="nav menu <?php echo $params->get('moduleclass_sfx'); ?> maximenuck" style="<?php echo $menubgcolor; ?>">
            <?php
			if ($logoimage) {
...
//    $stopdropdown = ($item->level > 120) ? '-nodrop' : '';
	...
	$nextcolumnstyles = isset($item->nextcolumnwidth) ? ' style="width:' . $item->nextcolumnwidth . 'px;float:left;"' : '';
    ...
    if (isset($item->content) AND $item->content) {
        echo '<li class="maximenuck maximenuckmodule' . $stopdropdownclass . $item->classe . ' level' . $item->level .' '.$item->liclass . '">' . $item->content;
		$item->ftitle = '';
    }
    ...
    if ($item->ftitle != "") {
		...
		if ($item->menu_image) {
			...
			if ($item->params->get('menu_text', 1 ) AND !$params->get('imageonly', '0')) {
				switch ($params->get('menu_images_align', 'default')) :
					default:
                                        case 'default':
        ...
        echo '<li class="maximenuck' . $stopdropdownclass . $item->classe . ' level' . $item->level .' '.$item->liclass . '" style="z-index : ' . $zindex . ';">';
        switch ($item->type) :
            default:
    ...
    if ($item->deeper) {
        ...
        if (isset($item->submenuswidth) || $item->leftmargin || $item->topmargin || $item->colbgcolor) {
            ...
            if (isset($item->submenucontainerheight) && $item->submenucontainerheight)
                $item->styles .= "height:" . $item->submenucontainerheight . "px;";
            $item->styles .= "\"";
        } else {
            $item->styles = "";
        }

        echo "\n\t<div class=\"floatck\" " . $item->styles . ">" . $close . "<div class=\"maximenuck2 first \" " . $nextcolumnstyles . ">\n\t<ul class=\"maximenuck2\">";
    ...
    elseif ($item->is_end) {
        echo str_repeat("</li>\n\t</ul>\n\t<div class=\"clr\"></div></div><div class=\"clr\"></div></div>", $item->level_diff);
        echo "</li>";
    }
