...
var GaForm = new Class({
	initialize: function(panels, toolbar, container){
		if( panels.length <=0 ){
			return ;	
		}
	 
		var tmptb = panels[0].getElement(".ga-toolbar-items") ;  
		toolbar.adopt(tmptb );
		var gapanel = [];
		for( var i=1; i< panels.length-1; i++ ) {
			container.adopt(panels[i]);	
			gapanel[i-1] = panels[i].setStyle("display","none").tween("opacity",0);
		}
		panels[0].empty().hide();
		panels[panels.length-1].hide();
		panels = null;
		// add event for switching panel configuration form
		gapanel[0].fade("in").setStyle("display","block");
		$$(".ga-toolbar-items > li")[0].addClass("active");
		$$(".ga-toolbar-items > li").each( function(item, i){
			item.addEvent( "click" , function(){
				$$(".ga-toolbar-items > li:.active").removeClass("active");
				item.addClass("active");										  
				if( $defined(gapanel[i]) ) {
					 gapanel[i].tween("opacity",1).setStyle("display","block");
					 for( var j=0; j< gapanel.length; j++ ) {
					 	if( j != i ){
							gapanel[j].tween("opacity",0).setStyle("display","none");
						}
					 }
				}
			} );
...
$(window).addEvent( 'load', function(){
	...
	$$('.gaswitch').each(function(el){
			...
			if(opts.length==2){
				...
				var switchClass = (el.value == val[0]) ? text[0] : text[1];
			
				var switcher = new Element('div',{'class' : 'ga-switcher-'+switchClass});

				switcher.inject(el, 'after');
				switcher.addEvent("click", function(){
					if(el.value == val[1]){
						switcher.setProperty('class','ga-switcher-'+text[0]);
						el.value = val[0];
					} else {
						switcher.setProperty('class','ga-switcher-'+text[1]);
						el.value = val[1];
					}
				});
		}
	...
	$$(".ga-cbktoggle").each( function(item){
		item.addEvent( 'click', function(){
			if(item.checked){
				$$("#toggle-"+item.id).show();
			} else {
